...
def save_scene(gridworld,trace): # save each scene in trace
    ...
    sys_snapshot = []
    sys_snapshot.append((gridworld.agent.name,gridworld.agent.x))
    ped_snapshot = []
    ped_snapshot.append((gridworld.tester.name, gridworld.tester.s))
    current_scene = Scene(gridworld.timestep, sys_snapshot, ped_snapshot)
